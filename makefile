...
ifeq ($(BAHTI_ANALYSIS), true)
	...
	CPPFLAGS := -Wno-deprecated -g -std=c++11 -DECpack=$(ECpack) -DGRAPHICS=$(GRAPHICS) -DMAXtime=$(MAXtime) -DBAHTI_ANALYSIS=$(BAHTI_ANALYSIS) -DANALYSIS_HYSTERESIS=$(ANALYSIS_HYSTERESIS) -DANALYSIS_TIME_TO_PATTERN=$(ANALYSIS_TIME_TO_PATTERN) -DANALYSIS_MAX_LENGTH=$(ANALYSIS_MAX_LENGTH) -DANALYSIS_FILO_LENGTHS=$(ANALYSIS_FILO_LENGTHS) #cherry
endif
